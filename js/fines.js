...
function classifyFineIssuer(f) {
  ...
  if (t === 'court' || t === 'judge') return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><rect width="4" height="18" x="10" y="3" rx="1"/><path d="M4 8h16"/><path d="M4 8l-2 6c0 1.1.9 2 2 2s2-.9 2-2l-2-6Z"/><path d="M20 8l-2 6c0 1.1.9 2 2 2s2-.9 2-2l-2-6Z"/></svg>', label: 'Суд' };
  if (t === 'admin' || t === 'administration') return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>', label: 'Адміністрація' };
  if (t === 'police' || t === 'nps') return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3 6 6 .5-4.5 4.5 1.5 6.5-6-3.5-6 3.5 1.5-6.5L4 8.5l6-.5z"/></svg>', label: 'НПС' };
  const s = (f.issuer || f.issuedBy || '').toLowerCase();
  if (s.includes('суд')) return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><rect width="4" height="18" x="10" y="3" rx="1"/><path d="M4 8h16"/><path d="M4 8l-2 6c0 1.1.9 2 2 2s2-.9 2-2l-2-6Z"/><path d="M20 8l-2 6c0 1.1.9 2 2 2s2-.9 2-2l-2-6Z"/></svg>', label: f.issuer || 'Суд' };
  if (s.includes('адмін')) return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>', label: f.issuer || 'Адміністрація' };
  if (s.includes('нпс') || s.includes('сержант') || s.includes('лейтенант') || s.includes('капітан')
    || s.includes('патруль') || s.includes('кадет')) return { icon: '<svg width="16" height="16" style="vertical-align:text-bottom" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3 6 6 .5-4.5 4.5 1.5 6.5-6-3.5-6 3.5 1.5-6.5L4 8.5l6-.5z"/></svg>', label: f.issuer || 'НПС' };
  return { icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:text-bottom"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>', label: f.issuer || f.issuedBy || '—' };
}
  ...
  if (P) {
    ...
    if (pKey && Array.isArray(P[pKey]) && P[pKey][0]) {
      ...
      if (cat.includes('адмін') || cat === 'адміністрація') {
        return {
          canIssueDirectly: true, canSubmitPending: true, canApprove: true,
...
function renderIssueFineForm() {
  ...
  const evidenceLabel = role.canIssueDirectly ? 'Доказ (PNG/JPG, опційно)' : 'Доказ (PNG/JPG) *';
...
async function submitFineForm() {
  ...
  if (!role.canIssueDirectly && !evidenceData) { if (status) status.textContent = 'Для штрафу на затвердження потрібен скрін доказу (PNG/JPG)'; return; }

  const proposerTg = (window.state && window.state.telegram && window.state.telegram.id) || (window.state && window.state.telegram && window.state.telegram.username) || null;
  ...
  if (role.canIssueDirectly) {
    ...
    if (targetTg && typeof window.addFineDirect === 'function') {
      ...
      const pending = Object.assign({}, fine, {
        ...
        proposedBy: proposerTg,
        proposedAt: new Date().toISOString(),
        rbxOnly: true
      });
      if (typeof window.submitPendingFine === 'function') ok = await window.submitPendingFine(pending);
    }
  } else {
    const pending = Object.assign({}, fine, {
...
async function renderFines() {
  ...
  const tgUserObj = (window.state && window.state.telegram) || (typeof tgUser !== 'undefined' ? tgUser : null);

  let fines = [];
  if (tgUserObj && typeof window.fetchProfile === 'function') {
    try {
      const profile = await window.fetchProfile(tgUserObj);
      if (profile && Array.isArray(profile.fines)) fines = profile.fines;
    } catch (e) {}
  }
  if (!fines.length && window.state && Array.isArray(window.state.fines)) fines = window.state.fines;
  ...
  if (typeof window.fetchPaymentRequests === 'function') {
    try {
      ...
      pendingPaymentIds = reqs
        .filter(r => r && (r.username === tgUserObj?.username || r.tgId === tgUserObj?.id))
        .map(r => r.fineId);
    } catch (e) {}
  }
...
async function renderPendingApprovalSection() {
  ...
  if (typeof window.fetchPendingFines === 'function') {
    try { items = await window.fetchPendingFines(); } catch (e) {}
  }
  ...
  root.innerHTML = `
    ...
    const targetStr = it.target ? `@${it.target}` : (it.targetRbx ? `Roblox: ${it.targetRbx}` : '—');
  ...
  root.innerHTML = `
    ...
    ${items.map(it => `
      <div class="fine-card fine-card-pending">
        ...
        <div class="fine-card-row"><span class="fine-card-label">Гравець:</span><span class="fine-card-value">${it.tgId || it.username || '—'}</span></div>
